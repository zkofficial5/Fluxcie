...
const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  // Build audio URL for voice messages
  const audioUrl =
    message.type === "voice" && message.filePath
      ? `http://localhost:8000/storage/${message.filePath}`
      : undefined;

  // Build image URL for images
  const imageUrl =
    message.type === "image" && message.filePath
      ? `http://localhost:8000/storage/${message.filePath}`
      : undefined;
      ...
      >
        ...
        >
          {!isSent && conversation?.isGroup && sender && (
            <p className="text-xs text-primary font-medium mb-1 ml-3">
              {sender.name}
            </p>
          )}
            ...
            {message.type === "voice" && message.voiceDuration ? (
              ...
              <div>
                <img
                  src={imageUrl}
                  alt="Shared image"
                  className="rounded-lg max-w-sm w-full h-auto mb-2"
                  onError={(e) => {
              ...
              <div>
                <a
                  href={`http://localhost:8000/storage/${message.filePath}`}
                  download={message.fileName}
                  className="text-sm underline hover:no-underline"
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  📎 {message.fileName} ({(message.fileSize! / 1024).toFixed(1)}{" "}
                  KB)
                </a>
                {message.content && !message.content.startsWith("📎") && (
